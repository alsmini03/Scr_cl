    ...
    <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-3 no-scrollbar">
          <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">드래그하여 순서를 변경하세요</p>
          {tabs.map((tab) => (
            <div
              key={tab.id}
              draggable
              ...
              className={cn(
                "flex items-center gap-3 p-4 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border transition-all active:scale-[0.98] cursor-move",
                draggedId === tab.id ? "opacity-40 border-primary scale-95" : "border-slate-100 dark:border-slate-700 hover:border-primary/30"
              )}
            >
              <span className="material-symbols-outlined text-slate-400 select-none">drag_indicator</span>
              <span className="flex-1 font-bold text-slate-700 dark:text-slate-200 truncate">{tab.name}</span>
              <button
                onClick={(e) => onDelete(tab.id, e)}
                className="size-8 flex items-center justify-center rounded-xl bg-red-50 dark:bg-red-900/20 text-red-500 hover:bg-red-500 hover:text-white transition-colors"
              >
